...
class FastSpeech2Model(SpectrogramGenerator):
    ...
    def __init__(self, cfg: DictConfig, trainer: 'Trainer' = None):
        ...
        #self.pe_weighted = cfg.pe_weighted
        #self.other_weight = cfg.other_weight
        ...
        self.emotion_predictor = SuperbPredictor()
    ...
    def mse_loss_weights(x, y, weights):
        return torch.mean(torch.pow(x-y, 2) * weights)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # vocoder = Vocoder.from_pretrained(pretrained_model, strict=True).eval().cuda()
        # wavs = vocoder.convert_spectrogram_to_audio(spec=mel) #might need to use mel.transpose(1, 2)
        # weights = torch.exp(self.emotion_predictor.predict_emotion_batch(wavs)[:,ANGRY_IDX])

        # if self.other_weight:
        #     total_loss = self.loss(
        #         spec_pred=mel.transpose(1, 2), spec_target=spec, spec_target_len=spec_len, pad_value=-11.52, weights=weights
        #     )
        # else:
        #     total_loss = self.loss(
        ...
        #         log_duration_pred=log_dur_preds, duration_target=durations.float(), mask=encoded_text_mask, weights=weights
        #     )
        # else:
        #     dur_loss = self.durationloss(
        ...
        if self.pitch:
            # if self.pe_weighted:
            #     pitch_loss = self.mse_loss_weights(pitch_preds, pitch, weights)
            # else:
        ...
        if self.energy:
            # if self.pe_weighted:
            #     energy_loss = self.mse_loss_weights(energy_preds, energy, weights)
            # else:
            #     energy_loss = self.mseloss(energy_preds, energy)
